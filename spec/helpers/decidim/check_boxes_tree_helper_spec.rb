...
module Decidim
  describe CheckBoxesTreeHelper do
    ...
    describe "#sorted_children_for" do
      let!(:participatory_space) { create(:participatory_process, organization: organization, scopes_enabled: true, scope: scope) }
      let!(:scope) { create(:scope, name: { en: "0. Scope" }, organization: organization) }
      let!(:scope_1) { create(:scope, name: { en: "1 Scope" }, organization: organization, parent_id: scope.id) }
      let!(:scope_2) { create(:scope, name: { en: "1.1 A Scope" }, organization: organization, parent_id: scope.id) }
      let!(:scope_3) { create(:scope, name: { en: "3.2 Scope" }, organization: organization, parent_id: scope.id) }
      let!(:scope_4) { create(:scope, name: { en: "3.1 Scope" }, organization: organization, parent_id: scope.id) }
      let!(:scope_5) { create(:scope, name: { en: "1.1 B Scope" }, organization: organization, parent_id: scope.id) }

      it "returns sorted children scopes" do
        expect(helper.sorted_children_for(participatory_space.scope)).to eq([scope_1, scope_2, scope_5, scope_4, scope_3])
      end
    end
  end
end
