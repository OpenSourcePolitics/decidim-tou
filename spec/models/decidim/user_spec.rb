...
module Decidim
  describe User do
    ...
    let(:user) { build(:user, organization: organization, registration_metadata: registration_metadata) }
    ...
    let!(:metropolis_work_area) { create(:scope, organization: organization) }
    let!(:metropolis_residential_area) { create(:scope, organization: organization) }
    ...
    describe "#fetch_registration_area" do
      context "when user has registration metadata areas" do
        it "returns scope name rather than scope id" do
          expect(subject.fetch_registration_area(:city_work_area)).to eq(translated(city_work_area.name))
          expect(subject.fetch_registration_area(:city_residential_area)).to eq(translated(city_residential_area.name))
          expect(subject.fetch_registration_area(:metropolis_residential_area)).to eq(translated(metropolis_residential_area.name))
          expect(subject.fetch_registration_area(:metropolis_work_area)).to eq(translated(metropolis_work_area.name))
        end
      end

      context "with unknown key" do
        it "returns empty string" do
          expect(subject.fetch_registration_area(:unknown_key)).to be_empty
        end
      end

      context "when area value is not an integer" do
        let(:registration_metadata) do
          {
            city_residential_area: "12"
          }
        end

        it "returns empty string" do
          expect(subject.fetch_registration_area(:city_residential_area)).to be_empty
        end
      end

      context "when scope doesn't exists for a given ID" do
        let(:registration_metadata) do
          {
            city_residential_area: 9999
          }
        end
