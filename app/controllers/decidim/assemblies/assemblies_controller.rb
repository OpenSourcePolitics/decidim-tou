...
module Decidim
  module Assemblies
    ...
    class AssembliesController < Decidim::Assemblies::ApplicationController
      ...
      # Returns : Decidim::ParticipatoryProcess::ActiveRecord_Relation || Array
      def sort_related_pp(participatory_processes)
        active_pps = []
        closed_pps = []
        participatory_processes&.each do |pp|
          if pp.active?
            active_pps << pp
          else
            closed_pps << pp
        ...
        return participatory_processes if (active_pps + closed_pps).uniq.blank?

        (active_pps + closed_pps).uniq
      end
    end
  end
end
